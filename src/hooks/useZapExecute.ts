...
import { GUNITED_ADDRESSES } from "src/constants/addresses";
...
import { SUNITED_ADDRESSES } from "../constants/addresses";
...
export const useZapExecute = () => {
  ...
  return useMutation<ContractReceipt, Error, ZapExecuteOptions>(
     ...
     */
    async ({ slippage, sellAmount, tokenAddress, minimumAmount, gUNITED }) => {
      ...
      const toToken = gUNITED
        ? GUNITED_ADDRESSES[chain.id as keyof typeof GUNITED_ADDRESSES]
        : SUNITED_ADDRESSES[chain.id as keyof typeof SUNITED_ADDRESSES];
      if (!toToken)
        throw new Error(
          t`Unable to fetch address for token (${gUNITED ? "gUNITED" : "sUNITED"}) on network ${chain.id}`,
        );
      ...
      const transaction = await contract.ZapStake(
        ...
        sellAmount,
        toToken,
        ethers.utils.parseUnits(minimumAmount, gUNITED ? 18 : 9),
        swapData.to,
        swapData.data,
        address,
        additionalOptions,
    ...
    },
    {
      ...
      onSuccess: (_data, variables) => {
        ...
        const keysToRefetch = [
          balanceQueryKey(address, SUNITED_ADDRESSES, NetworkId.MAINNET),
          balanceQueryKey(address, GUNITED_ADDRESSES, NetworkId.MAINNET),
