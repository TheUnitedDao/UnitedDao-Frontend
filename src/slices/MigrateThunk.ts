...
import { GUNITED_ADDRESSES, MIGRATOR_ADDRESSES } from "src/constants/addresses";
...
const chooseContract = (token: string, networkID: NetworkId, signer: Signer): IERC20 => {
  let address: string;
  if (token === "ohm") {
    address = addresses[networkID].UNITED_ADDRESS;
  } else if (token === "sohm") {
    address = addresses[networkID].SUNITED_ADDRESS;
  } else if (token === "wsohm") {
    address = addresses[networkID].WSUNITED_ADDRESS;
  } else if (token === "gohm") {
    address = GUNITED_ADDRESSES[networkID as keyof typeof GUNITED_ADDRESSES];
  } else {
...
export const bridgeBack = createAsyncThunk(
  ...
  async ({ provider, address, networkID, value }: IValueAsyncThunk, { dispatch }) => {
    ...
    try {
      ...
      const text = `Bridge Back gUNITED`;
      ...
      if (unMigrateTx) {
        ...
        dispatch(info("Successfully unwrapped gUNITED!"));
      }
    } catch (e: unknown) {
      dispatch(error((e as IJsonRPCError).message));
    } finally {
...
export const migrateSingle = createAsyncThunk(
  ...
  async ({ provider, address, networkID, type, amount, gUNITED, signer }: IMigrateSingleAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
      return;
    }
    ...
    try {
      migrateTx = await migrator.migrate(
        ...
        type,
        gUNITED ? TokenType.WRAPPED : TokenType.STAKED,
      );
...
export const migrateAll = createAsyncThunk(
  "migrate/migrateAll",
  async ({ provider, address, networkID, gUNITED, signer }: IMigrateAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
      return;
    }
    ...
    try {
      migrateAllTx = await migrator.migrateAll(gUNITED ? TokenType.WRAPPED : TokenType.STAKED);
