...
import { GUNITED_ADDRESSES, MIGRATOR_ADDRESSES, STAKING_ADDRESSES } from "src/constants/addresses";
import { Environment } from "src/helpers/environment/Environment/Environment";
import { RootState } from "src/store";
import { IERC20, IERC20__factory, WsUNITED } from "src/typechain";
import { GUNITED__factory } from "src/typechain/factories/GUNITED__factory";

import ierc20Abi from "../abi/IERC20.json";
import MockSohm from "../abi/MockSohm.json";
import wsUNITED from "../abi/wsUNITED.json";
  ...
  async ({ address, networkID, provider }: IBaseAddressAsyncThunk): Promise<IUserBalances> => {
    ...
    try {
      const wsohmContract = new ethers.Contract(
        addresses[networkID].WSUNITED_ADDRESS as string,
        wsUNITED.abi,
        provider,
      ) as WsUNITED;
    ...
    try {
      const ohmContract = new ethers.Contract(
        addresses[networkID].UNITED_ADDRESS as string,
        ierc20Abi.abi,
        provider,
      ) as IERC20;
    ...
    try {
      const sohmContract = new ethers.Contract(
        addresses[networkID].SUNITED_ADDRESS as string,
        ierc20Abi.abi,
        provider,
      ) as IERC20;
    ...
    /*
      Needed a sUNITED contract on testnet that could easily
      be manually rebased to test redeem features
    */
    try {
      if (Environment.isGiveEnabled() && addresses[networkID] && addresses[networkID].MOCK_SUNITED) {
        const mockSohmContract = new ethers.Contract(
          addresses[networkID].MOCK_SUNITED as string,
          MockSohm.abi,
          provider,
        ) as IERC20;
        mockSohmBalance = await mockSohmContract.balanceOf(address);
      } else {
        console.debug("Unable to find MOCK_SUNITED contract on chain ID " + networkID);
      }
    } catch (e) {
      handleContractError(e);
    }
...
export const getMigrationAllowances = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    if (addresses[networkID].UNITED_ADDRESS) {
      try {
        const ohmContract = IERC20__factory.connect(addresses[networkID].UNITED_ADDRESS, provider);
    ...
    if (addresses[networkID].SUNITED_ADDRESS) {
      try {
        const sOhmContract = IERC20__factory.connect(addresses[networkID].SUNITED_ADDRESS, provider);
    ...
    if (addresses[networkID].WSUNITED_ADDRESS) {
      try {
        const wsOhmContract = IERC20__factory.connect(addresses[networkID].WSUNITED_ADDRESS, provider);
    ...
    if (GUNITED_ADDRESSES[networkID as keyof typeof GUNITED_ADDRESSES]) {
      try {
        const gOhmContract = IERC20__factory.connect(
          GUNITED_ADDRESSES[networkID as keyof typeof GUNITED_ADDRESSES],
          provider,
        );
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk, { dispatch }) => {
    ...
    try {
      const gOhmContract = GUNITED__factory.connect(
        GUNITED_ADDRESSES[networkID as keyof typeof GUNITED_ADDRESSES],
        provider,
      );
