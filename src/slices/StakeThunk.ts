...
import { UNITED_ADDRESSES, SUNITED_ADDRESSES, STAKING_ADDRESSES } from "src/constants/addresses";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID, version2 }: IChangeApprovalWithVersionAsyncThunk, { dispatch }) => {
    ...
    const ohmV2Contract = new ethers.Contract(
      UNITED_ADDRESSES[networkID as keyof typeof UNITED_ADDRESSES] as string,
      ierc20ABI.abi,
      signer,
    ) as IERC20;
    const sohmV2Contract = new ethers.Contract(
      SUNITED_ADDRESSES[networkID as keyof typeof SUNITED_ADDRESSES] as string,
      ierc20ABI.abi,
      signer,
    ) as IERC20;
    let approveTx;
