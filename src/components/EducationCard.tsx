...
export function LargeWallet() {
  return (
    <LargeGraphic
      icon={<Icon name="vault-wallet" fontSize="large" />}
      title={t`Wallet`}
      subtitle={t`Deposit sUNITED from wallet`}
      subtext={t`United Give is a means of directing the yield that is accrued on your sUNITED to another wallet. The first step is depositing your sUNITED and specifying a recipient.`}
    />
  );
}
...
export function LargeVault() {
  return (
    <LargeGraphic
      icon={<Icon name="vault-lock" fontSize="large" />}
      title={t`Vault`}
      subtitle={t`Lock sUNITED in vault`}
      subtext={t`Then, your deposited sUNITED is kept in a vault smart contract that will send your rebases to the recipient.
      You can withdraw or edit your principal sUNITED amount at any time.`}
    />
  );
}
