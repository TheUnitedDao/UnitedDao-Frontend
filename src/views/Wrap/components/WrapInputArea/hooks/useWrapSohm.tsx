...
export const useWrapSohm = () => {
  ...
  const balance = useBalance(SUNITED_ADDRESSES)[networks.MAINNET].data;
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      if (_amount.gt(balance)) throw new Error(t`You cannot wrap more than your sUNITED balance`);

      if (!contract) throw new Error(t`Please switch to the Ethereum network to wrap your sUNITED`);
    ...
    },
    {
      ...
      onSuccess: async (_, amount) => {
        ...
        dispatch(createInfoToast(t`Successfully wrapped sUNITED to gUNITED`));
      },
    },
  );
};
