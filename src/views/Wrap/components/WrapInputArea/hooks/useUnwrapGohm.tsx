...
export const useUnwrapGohm = () => {
  ...
  const balance = useBalance(GUNITED_ADDRESSES)[networks.MAINNET].data;
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      if (_amount.gt(balance)) throw new Error(t`You cannot unwrap more than your gUNITED balance`);

      if (!contract) throw new Error(t`Please switch to the Ethereum network to unwrap your gUNITED`);
    ...
    },
    {
      ...
      onSuccess: async (_, amount) => {
        trackGAEvent({
          category: "Wrapping",
          action: "Unwrap gUNITED",
          value: new DecimalBigNumber(amount, 18).toApproxNumber(),
        });

        const keysToRefetch = [
          balanceQueryKey(address, SUNITED_ADDRESSES, networks.MAINNET),
          balanceQueryKey(address, GUNITED_ADDRESSES, networks.MAINNET),
        ];
        ...
        dispatch(createInfoToast(t`Successfully unwrapped from gUNITED to sUNITED`));
      },
    },
  );
};
