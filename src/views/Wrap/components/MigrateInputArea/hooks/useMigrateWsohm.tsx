...
import { GUNITED_ADDRESSES, MIGRATOR_ADDRESSES, WSUNITED_ADDRESSES } from "src/constants/addresses";
...
export const useMigrateWsohm = () => {
  ...
  const balances = useBalance(WSUNITED_ADDRESSES);
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      if (_amount.gt(balance)) throw new Error(t`You cannot migrate more than your wsUNITED balance`);
    ...
    },
    {
      ...
      onSuccess: async (_, amount) => {
        trackGAEvent({
          category: "Migration",
          action: "Migrate wsUNITED",
          value: new DecimalBigNumber(amount, 18).toApproxNumber(),
        });

        const keysToRefetch = [
          balanceQueryKey(address, WSUNITED_ADDRESSES, chain.id),
          balanceQueryKey(address, GUNITED_ADDRESSES, chain.id),
        ];
        ...
        dispatch(createInfoToast(t`Successfully migrated from wsUNITED to gUNITED`));
      },
    },
  );
};
