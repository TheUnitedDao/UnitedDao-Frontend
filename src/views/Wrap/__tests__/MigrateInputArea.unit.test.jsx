...
describe("Wrap Input Area", () => {
  it("Should Render Input when has Token Approval", async () => {
    fireEvent.change(await screen.findByPlaceholderText("Enter an amount of wsUNITED"), { target: { value: "1" } });
    expect(screen.getByTestId("migrate-button"));
    expect(container).toMatchSnapshot();
  });
});

describe("Check Migrate to gUNITED Error Messages", () => {
  it("Should error when not on Avalanche or Arbitrum", async () => {
    WAGMI.useNetwork = jest.fn(() => {
      return {
        chain: {
          id: 137, //polygon isnt supported for wsUNITED to gUNITED
        },
      };
    });
  ...
  it("Error message amount > wallet balance", async () => {
    fireEvent.change(await screen.findByPlaceholderText("Enter an amount of wsUNITED"), { target: { value: "10000" } });
    fireEvent.click(screen.getByTestId("migrate-button"));
    expect(await screen.findByText("You cannot migrate more than your wsUNITED balance")).toBeInTheDocument();
  });
