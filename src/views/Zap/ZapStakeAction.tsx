...
const ZapStakeAction: React.FC = () => {
  ...
  const [outputGUNITED, setOutputGUNITED] = useState<boolean | null>(null);
  const handleSelectOutputToken = (token: string) => {
    if (token === "gUNITED") {
      setOutputGUNITED(true);
    } else if (token === "sUNITED") {
      setOutputGUNITED(false);
    }
  ...
  const exchangeRate: number | null = useMemo(() => {
    if (outputGUNITED != null && selectedTokenBalance && ohmMarketPrice.data && gOhmMarketPrice.data) {
      return (
        (outputGUNITED === undefined || outputGUNITED === null || outputGUNITED
          ? gOhmMarketPrice.data
          : ohmMarketPrice.data) / selectedTokenBalance.price
      );
    } else {
      return null;
    }
  }, [outputGUNITED, ohmMarketPrice, gOhmMarketPrice, selectedTokenBalance]);
  ...
  const onZap = async () => {
    if (outputGUNITED != null && selectedTokenBalance) {
      zapExecute.mutate({
  ...
  return (
    <Root>
                  ...
                  <Box flexDirection="column" display="flex">
                    <Box flexDirection="row" display="flex" alignItems="center" justifyContent="flex-end">
                      <ButtonBase onClick={handleOutputOpen}>
                        <Token name={outputGUNITED ? "wsUNITED" : "sUNITED"} />
                        <Box width="10px" />
                        <Typography>{outputGUNITED ? "gUNITED" : "sUNITED"}</Typography>
                        {downIcon}
                      </ButtonBase>
                    </Box>
                    <Box flexDirection="row" display="flex" alignItems="center">
                      <Typography color="textSecondary">{`Balance ${formatBalance(
                        outputGUNITED ? gOhmBalance : sOhmBalance,
                      )}`}</Typography>
                    </Box>
                  </Box>
                </div>
      ...
      <Box justifyContent="space-between" flexDirection="row" display="flex" width="100%" marginY="8px">
        ...
        <Typography>
          {outputGUNITED == null || !selectedTokenBalance
            ? "nil"
            : `${trim(exchangeRate || 0, 4)} ${selectedTokenBalance.symbol}`}{" "}
          = 1 {outputGUNITED ? "gUNITED" : "sUNITED"}
        </Typography>
      </Box>
      <Box
        justifyContent="space-between"
      ...
      >
        ...
        <Typography>
          {minimumAmountString} {outputGUNITED ? "gUNITED" : "sUNITED"}
        </Typography>
      </Box>
      {hasTokenAllowance ? (
        <Button
          ...
          disabled={
            ...
            DISABLE_ZAPS ||
            (+outputQuantity < 0.5 && !outputGUNITED)
          }
          onClick={onZap}
        >
          {zapExecute.isLoading ? (
