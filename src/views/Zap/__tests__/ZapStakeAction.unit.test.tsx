...
describe("<ZapStakeAction/> ", () => {
  ...
  it("gUNITED should autopopulate with correct value based on ETH input", async () => {
    render(
      <>
        <ZapStakeAction />
      </>,
    ...
    fireEvent.click(await screen.getAllByText("gUNITED")[0]);
  ...
  it("sUNITED should autopopulate with correct value based on ETH input", async () => {
    //@ts-expect-error
    ContractAllowance.useContractAllowance = jest.fn().mockReturnValue({ data: BigNumber.from(10) });
    render(
      <>
    ...
    expect(await screen.findByText("Enter Amount"));
    fireEvent.click(await screen.findByTestId("zap-output"));
    fireEvent.click(await screen.getAllByText("sUNITED")[0]);
