...
export const fetchBond = async ({ id, isInverseBond, networkId }: UseBondOptions) => {
  ...
  const quoteToken = isInverseBond ? UNITED_TOKEN : await getTokenByAddress({ address: market.quoteToken, networkId });
   ...
   *
   * this is either the number of UNITED that the market can sell
   ...
   * interval. for example, if capacity is 1,000 UNITED, there are 10 days to conclusion,
   * and the preferred deposit interval is 1 day, max payout would be 100 UNITED.
   */
