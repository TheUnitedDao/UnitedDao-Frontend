...
beforeEach(async () => {
  Token.UNITED_TOKEN.getPrice = jest.fn().mockResolvedValue(new DecimalBigNumber("20"));
  Token.DAI_TOKEN.getPrice = jest.fn().mockResolvedValue(new DecimalBigNumber("1"));
  Token.UNITED_DAI_LP_TOKEN.getPrice = jest.fn().mockResolvedValue(new DecimalBigNumber("200000"));
...
afterEach(() => {
  ...
  Token.UNITED_TOKEN.getPrice.mockReset();
  Token.DAI_TOKEN.getPrice.mockReset();
  Token.UNITED_DAI_LP_TOKEN.getPrice.mockReset();
...
describe("Bonds", () => {
  ...
  it("should render component with UNITED-DAI LP", async () => {
    render(<Bond />);

    expect(await screen.queryAllByText("UNITED-DAI LP")[0]).toBeInTheDocument();
  });
...
describe("Bond Modal", () => {
  ...
  it("Should Return Error when negative amount is entered", async () => {
    ...
    fireEvent.change(await screen.findByPlaceholderText("Enter an amount of UNITED-DAI LP"), {
      target: { value: "-1" },
    });
    fireEvent.click(await screen.findByText("Bond"));
  ...
  it("Should Return Error when amount is greater than balance", async () => {
    ...
    fireEvent.click(await screen.findByText("Bond"));
    expect(await screen.findByText("You cannot bond more than your UNITED-DAI LP balance")).toBeInTheDocument();
  });
  ...
  it("Return Error when Amount is > Max Payout", async () => {
    ...
    fireEvent.click(await screen.findByText("Bond"));
    expect(
      await screen.findByText("The maximum you can bond at this time is 0.348287073676420851 UNITED-DAI LP"),
    ).toBeInTheDocument();
  });
  ...
  it("Should Execute Successfully", async () => {
    ...
    fireEvent.click(await screen.findByText("Bond"));
    expect(await screen.findByText("Successfully bonded UNITED-DAI LP")).toBeInTheDocument();
  });
});
