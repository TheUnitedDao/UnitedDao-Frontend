  ...
  return (
    <Paper headerText="Your Bonds">
      <Box display="flex" flexDirection="column" alignItems="center">
        ...
        >
          <Tab aria-label="payout-sohm-button" label="sUNITED" style={{ fontSize: "1rem" }} />
          <Tab aria-label="payout-sohm-button" label="gUNITED" style={{ fontSize: "1rem" }} />
        </Tabs>
      </Box>
        ...
        <Box display="flex" flexDirection="column" alignItems="center" mt="24px" width={isSmallScreen ? "100%" : "50%"}>
          ...
          <Box mt="4px" mb="8px">
            <Typography variant="h4" align="center">
              {isPayoutGohm
                ? `${totalClaimableBalance.toString({ decimals: 4, format: true })} gUNITED`
                : `${currentIndex?.mul(totalClaimableBalance).toString({ decimals: 4, format: true })} sUNITED`}
            </Typography>
          </Box>
            ...
            {notes.map(note => (
              <Box mt="32px">
                ...
                <Box display="flex" justifyContent="space-between" mt="16px">
                  ...
                  <Typography>
                    {isPayoutGohm
                      ? `${note.payout.toString({ decimals: 4, format: true })} gUNITED`
                      : `${currentIndex?.mul(note.payout).toString({ decimals: 4, format: true })} sUNITED`}
                  </Typography>
                </Box>
