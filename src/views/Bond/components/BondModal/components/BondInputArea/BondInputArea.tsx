...
export const BondInputArea: React.VFC<{
  ...
  ${isInverseBond ? props.bond.baseToken.name : `sUNITED`}`;
            ...
            <TokenAllowanceGuard
              ...
              message={
                <>
                  ...
                  <Trans>Please approve United DAO to use your</Trans> <b>{props.bond.quoteToken.name}</b>{" "}
      ...
      <Box mt="24px">
          ...
          balance={
            <span>
              ...
              {!isInverseBond && !!currentIndex && (
                <span>
                  (≈{amountInBaseToken.div(currentIndex).toString({ decimals: 4, format: true, trim: false })} gUNITED)
                </span>
              )}
            </span>
          }
          tooltip={t`The total amount of payout asset you will recieve from this bond purchase. (sUNITED quantity will be higher due to rebasing)`}
        />
        ...
        <DataRow
          ...
          tooltip={t`Negative discount is bad (you pay more than the market value). The bond discount is the percentage difference between ${
            isInverseBond ? props.bond.baseToken.name : `UNITED`
