  ...
  return useQuery<number, Error>(
    ...
    async () => {
      ...
      const isFirstTokenGohm =
        ...
        GUNITED_ADDRESSES[pool.networkID as keyof typeof GUNITED_ADDRESSES]?.toLowerCase();
...
export const BalancerPoolTVL = (pool: ExternalPool) => {
  ...
  const { data, isFetched, isLoading } = useQuery(
    ...
    async () => {
      ...
      const isFirstTokenGohm =
        vaultBalances.tokens[0] === GUNITED_ADDRESSES[pool.networkID as keyof typeof GUNITED_ADDRESSES]?.toLowerCase();
      const firstTokenPrice = isFirstTokenGohm ? gohmPrice : nonGohmTokenPrice;
      const secondTokenPrice = isFirstTokenGohm ? nonGohmTokenPrice : gohmPrice;
      const totalLpAsUSD =
