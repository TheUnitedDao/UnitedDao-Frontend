...
describe("Check Stake to sUNITED Error Messages", () => {
  it("Error message with no amount", async () => {
    fireEvent.click(screen.getByText("Stake to sUNITED"));
  ...
  it("Error message amount > balance", async () => {
    fireEvent.input(await screen.findByRole("textbox"), { target: { value: "100" } });
    fireEvent.click(screen.getByText("Stake to sUNITED"));
    expect(await screen.findByText("You cannot stake more than your UNITED balance")).toBeInTheDocument();
  });
...
describe("Check Unstake sUNITED Error Messages", () => {
  beforeEach(() => {
    fireEvent.click(screen.getByText("Unstake"));
  });
  it("Error message with no amount", async () => {
    fireEvent.click(screen.getByText("Unstake sUNITED"));
    expect(await screen.findByText("Please enter a number")).toBeInTheDocument();
  });
  ...
  it("Error message with amount <=0 gUNITED", async () => {
    fireEvent.click(await screen.findByRole("checkbox"));
    fireEvent.input(await screen.findByRole("textbox"), { target: { value: "-1" } });
    fireEvent.click(screen.getByText("Unstake gUNITED"));
    expect(screen.getAllByText("Please enter a number greater than 0")[0]).toBeInTheDocument();
  });

  it("Error message amount > balance sUNITED", async () => {
    fireEvent.input(await screen.findByRole("textbox"), { target: { value: "11" } });
    fireEvent.click(screen.getByText("Unstake sUNITED"));
    expect(await screen.findByText("You cannot unstake more than your sUNITED balance")).toBeInTheDocument();
  });

  it("Error message amount > balance gUNITED", async () => {
    fireEvent.click(await screen.findByRole("checkbox"));
    fireEvent.input(await screen.findByRole("textbox"), { target: { value: "11" } });
    fireEvent.click(screen.getByText("Unstake gUNITED"));
    expect(await screen.findByText("You cannot unstake more than your gUNITED balance")).toBeInTheDocument();
  });
