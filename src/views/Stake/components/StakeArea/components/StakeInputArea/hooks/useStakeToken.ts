...
export const useStakeToken = (toToken: "sUNITED" | "gUNITED") => {
  const dispatch = useDispatch();
  const client = useQueryClient();
  const { address = "" } = useAccount();
  const networks = useTestableNetworks();
  const balance = useBalance(UNITED_ADDRESSES)[networks.MAINNET].data;
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      if (_amount.gt(balance)) throw new Error(t`You cannot stake more than your UNITED balance`);

      if (!contract) throw new Error(t`Please switch to the Ethereum network to stake your UNITED`);

      if (!address) throw new Error(t`Please refresh your page and try again`);

      const shouldRebase = toToken === "sUNITED";
    ...
    },
    {
      ...
      onSuccess: async (tx, amount) => {
        ...
        const keysToRefetch = [
          balanceQueryKey(address, UNITED_ADDRESSES, networks.MAINNET),
          balanceQueryKey(address, toToken === "sUNITED" ? SUNITED_ADDRESSES : GUNITED_ADDRESSES, networks.MAINNET),
        ];
        ...
        dispatch(createInfoToast(t`Successfully staked UNITED`));
      },
    },
  );
};
