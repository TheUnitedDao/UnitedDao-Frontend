...
export const useUnstakeToken = (fromToken: "sUNITED" | "gUNITED") => {
  ...
  const addresses = fromToken === "sUNITED" ? SUNITED_ADDRESSES : GUNITED_ADDRESSES;
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      const _amount = new DecimalBigNumber(amount, fromToken === "gUNITED" ? 18 : 9);
      ...
      const shouldRebase = fromToken === "sUNITED";
    ...
    },
    {
      ...
      onSuccess: async (tx, amount) => {
        trackGAEvent({
          ...
          value: new DecimalBigNumber(amount, fromToken === "gUNITED" ? 18 : 9).toApproxNumber(),
