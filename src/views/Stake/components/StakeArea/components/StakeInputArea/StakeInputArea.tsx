...
import { GUNITED_ADDRESSES, UNITED_ADDRESSES, SUNITED_ADDRESSES, STAKING_ADDRESSES } from "src/constants/addresses";
import { useBalance } from "src/hooks/useBalance";
import { useTestableNetworks } from "src/hooks/useTestableNetworks";
import { useLiveBonds } from "src/views/Bond/hooks/useLiveBonds";

import { GUNITEDConversion } from "./components/GUNITEDConversion";
...
export const StakeInputArea: React.FC<{ isZoomed: boolean }> = props => {
  ...
  const [stakedAssetType, setStakedAssetType] = useState<"sUNITED" | "gUNITED">("sUNITED");
  const [currentAction, setCurrentAction] = useState<"STAKE" | "UNSTAKE">("STAKE");

  const fromToken = currentAction === "STAKE" ? "UNITED" : stakedAssetType;

  // Max balance stuff
  const [amount, setAmount] = useState("");
  const addresses =
    fromToken === "UNITED" ? UNITED_ADDRESSES : fromToken === "sUNITED" ? SUNITED_ADDRESSES : GUNITED_ADDRESSES;
  ...
  return (
    <StyledBox mb={3}>
      {currentAction === "UNSTAKE" && liveInverseBonds && (
        <InfoNotification>
          {t`Unstaking your UNITED? Trade for Treasury Stables with no slippage & zero trading fees via`}
          &nbsp;
      ...
      <Box my={2}>
        <TokenAllowanceGuard
          ...
          message={
            currentAction === "STAKE" ? (
              <>
                <Trans>First time staking</Trans> <b>UNITED</b>?
                <br />
                <Trans>Please approve United DAO to use your</Trans> <b>UNITED</b> <Trans>for staking</Trans>.
              </>
            ) : (
              <>
                <Trans>First time unstaking</Trans> <b>{fromToken}</b>?
                <br />
                <Trans>Please approve United DAO to use your</Trans> <b>{fromToken}</b> <Trans>for unstaking</Trans>.
              </>
            )
          }
        >
      ...
      <Paper className="ohm-card confirm-dialog">
        <Box display={[null, "flex"]} alignItems="center" justifyContent="space-between">
          <Grid component="label" container alignItems="center" spacing={1} wrap="nowrap">
            <Grid item>sUNITED</Grid>

            <Grid item>
              <Switch
                disabled={isMutating}
                className="stake-to-ohm-checkbox"
                checked={stakedAssetType === "gUNITED"}
                inputProps={{ "aria-label": "stake to gohm" }}
                onChange={(_, checked) => setStakedAssetType(checked ? "gUNITED" : "sUNITED")}
              />
            </Grid>
          ...
          <Box marginTop={[2, 0]} flexShrink={0}>
            {stakedAssetType === "gUNITED" && <GUNITEDConversion amount={amount} action={currentAction} />}
          </Box>
        </Box>
      </Paper>
    </StyledBox>
