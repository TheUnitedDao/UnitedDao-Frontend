...
describe("<Stake/>", () => {
  ...
  it("should render all supported multi chain staking contracts", async () => {
    render(<Stake />);
    expect(await screen.getByText("gUNITED-AVAX")).toBeInTheDocument();
    ...
    const sushiContracts = await screen.findAllByText("gUNITED-wETH");
    expect(sushiContracts).toHaveLength(3);
    expect(await screen.getByText("gUNITED-FTM")).toBeInTheDocument();
