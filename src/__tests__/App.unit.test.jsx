...
describe("<App/>", () => {
  it("should render component", () => {
    disconnectedWallet();
    renderRoute("/");
    expect(screen.getByText("Connect your wallet to stake UNITED")).toBeInTheDocument();
  });
...
describe("Account Balances Slice", () => {
  ...
  it("should load Account Balances with no error", async () => {
    connectWallet();
    Contract.GUNITED__factory.connect = jest.fn().mockReturnValue({
...
describe("Staging Notification Checks", () => {
  ...
  it("Should display a notification banner when hostname = staging.uniteddao.global", async () => {
    connectWallet();
    render(<App />);
...
describe("Production Notification Check", () => {
  ...
  it("Should not display a notification when hostname not staging.uniteddao.global", async () => {
    connectWallet();
    render(<App />);
    expect(
