[![Lighthouse PWA Test](https://github.com/UnitedDAO/olympus-frontend/actions/workflows/lighthouse.yml/badge.svg)](https://github.com/UnitedDAO/olympus-frontend/actions/workflows/lighthouse.yml)
[![Contributor Covenant](https://img.shields.io/badge/Contributor%20Covenant-2.1-4baaaa.svg)](CODE_OF_CONDUCT.md)
[![UNITED Discord](https://img.shields.io/badge/chat-on%20discord-7289DA.svg)](https://discord.gg/UnitedDAO)
...
# [Ω United Frontend](https://app.uniteddao.global/)

This is the front-end repo for United that allows users to be part of the future of _Meta Greece_.
...
```bash
$ git clone https://github.com/UnitedDAO/olympus-frontend.git
$ cd olympus-frontend
...
[Here is an example](src/views/Stake/__tests__/Stake.unit.test.tsx) snapshot test and [here is the correspoding recorded snapshot](https://github.com/UnitedDAO/olympus-frontend/blob/develop/src/views/Stake/__tests__/__snapshots__/Stake.unit.test.tsx.snap). Keep in mind that for snapshot tests to be meaningful, they have to pre-populate components with variety of data sets (realistic, edge case, invalid).
...
- to retrieve test sUNITEDv1 click `Connect to Web3` and use function #3: `dripSUNITED`.
- After connecting to web3, click `Write` to execute and 10 sUNITED will automatically be transferred to your connected wallet.

_Note_: The faucet is limited to one transfer per wallet every 6500 blocks (~1 day)

_Note_: This faucet drips sUNITED v1 tokens. If you need to test v2 token flows (sUNITED, UNITED, gUNITED), you will first need to use the migration steps in the UI to convert from sUNITED v1 to sUNITED v2.
...
4. _market: `[10000000000000000000000000,60000000000,1000000]` <- [capacity (in UNITED or quote), initial price (9 decimals), debt buffer (3 decimals)]
...
2. _token: `0xbC9eE0D911739cBc72cd094ADA26F56E0C49EeAE` <- this is the payout token, the deposit token will be UNITED
...
2. In your gitpod dashboard, set a new variable named `GITHUB_UNITED_PERSONAL_ACCESS_TOKEN` to the value of the access token.
3. Use the button below to start a pre-configured gidpod environment.

[![Open in Gitpod](https://gitpod.io/button/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/UnitedDAO/olympus-frontend)
...
United uses [linguijs](https://github.com/lingui/js-lingui) to manage translation.

The language files are located in a submodule deployed in `src/locales/translations`. This submodule points to the [olympus translation repository](https://github.com/UnitedDAO/olympus-translations)
...
```JSX
{t`Your current Staked Balance is ${getSOhmBalance().toFixed(2)} sUNITED`}
```
...
| Branch | URL |
| --- | --- |
| master | <https://app.uniteddao.global> |
| deploy | <https://staging.uniteddao.global> |
...
We keep an updated list of bugs/feature requests in [Github Issues](https://github.com/UnitedDAO/olympusdao/issues).

![GitHub issues](https://img.shields.io/github/issues/olympusdao/olympusdao?style=flat-square)

Filter by ["good first issue"](https://github.com/UnitedDAO/olympusdao/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22) to get your feet wet!
...
- [Join our Discord](https://discord.gg/UnitedDAO) and ask how you can get involved with the DAO!
